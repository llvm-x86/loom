...
describe("GrepTool internal URL resolution", () => {
	...
	it("expands loom:// root to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# loom://tools/grep.md");
	...
	it("expands omp:// root to grep embedded documentation files", async () => {
		...
		// Legacy alias: omp:// input still resolves to the doc set, but emitted
		// paths use the canonical loom:// scheme.
		const result = await tool.execute("test-call", {
			pattern: "Grep file contents with a regex across files",
			path: "omp://",
		});
	...
	it("expands omp://docs to grep embedded documentation files", async () => {
		...
		const text = getResultText(result);
		expect(text).toContain("# loom://tools/read.md");
