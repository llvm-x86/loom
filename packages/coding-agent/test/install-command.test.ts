/**
 * Regression test for #1496 (bug 2): `loom install ./my-extension` used to be
...
describe("install command is registered as a top-level subcommand", () => {
	...
	test("CLI runner rejects only bare reserved management words", () => {
		expect(resolveCliArgv(["extensions"])).toEqual({
			error: '`loom extensions` is not a management command. Use `loom plugin list` / `loom plugin install`, or run `loom launch extensions` if you meant to send "extensions" as a prompt.',
		});
