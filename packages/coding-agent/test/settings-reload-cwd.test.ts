	...
	describe("project layer (on disk)", () => {
		...
		it("keeps failed project writes bound to their original cwd", async () => {
			...
			try {
				...
				expect(await Bun.file(path.join(bareProject, ".loom", "config.yml")).exists()).toBe(false);
			} finally {
				mkdirSpy.mockRestore();
			}

			await settings.flush();
			expect(YAML.parse(await Bun.file(path.join(startDir, ".loom", "config.yml")).text())).toEqual({
		...
		it("writes project model roles only to the project YAML", async () => {
			const settings = await Settings.init({ cwd: startDir, agentDir });
			const projectConfigPath = path.join(startDir, ".loom", "config.yml");
		...
		it("does not copy unedited roles from other project settings providers", async () => {
			await Bun.write(
				path.join(scopedProject, ".loom", "settings.json"),
			...
			expect(YAML.parse(await Bun.file(path.join(scopedProject, ".loom", "config.yml")).text())).toEqual({
