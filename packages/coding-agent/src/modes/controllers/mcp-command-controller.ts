...
export class MCPCommandController {
	...
	async #resolveOAuthEndpointsFromServer(config: MCPServerConfig): Promise<OAuthEndpoints> {
		...
		if (config.type !== "http" && config.type !== "sse") {
			...
			throw new Error(
				usesMcpRemote
					? `this server proxies OAuth through mcp-remote, which caches tokens machine-wide in ~/.mcp-auth (shared across every loom profile). Clear ~/.mcp-auth to force a fresh login, or replace the proxy with ${httpHint} so loom manages OAuth per profile.`
					: `stdio servers manage their own credentials, so loom has no OAuth to reauthorize. If the service supports OAuth over HTTP, configure it as ${httpHint} instead.`,
			);
		}
