/**
 * `loom install <target>` — top-level convenience over `omp plugin install` /
 * `loom plugin link`.
 *
 * The docs (omp.sh/docs/extension-authoring) advertise
 *
 *   loom install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `loom install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: loom install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
