...
export const commands: CommandEntry[] = [
	...
	{ name: "sync-context", load: () => import("./commands/sync-context").then(m => m.default) },
	...
	{ name: "scratch", load: () => import("./commands/scratch").then(m => m.default) },
	{ name: "webbridge", load: () => import("./commands/webbridge").then(m => m.default) },
	{ name: "search", load: () => import("./commands/web-search").then(m => m.default), aliases: ["q"] },
];

// Documented-looking plugin/marketplace verbs that are NOT registered top-level
// commands. Without a guard `resolveCliArgv` rewrites e.g. `loom marketplace add
// xyz` to `loom launch marketplace add xyz`, silently forwarding the argv to the
...
// The real commands live under `loom plugin <action>`; each entry maps a verb to
...
const RESERVED_TOP_LEVEL_WORDS: Record<string, string> = {
	extensions:
		'`loom extensions` is not a management command. Use `loom plugin list` / `loom plugin install`, or run `loom launch extensions` if you meant to send "extensions" as a prompt.',
	list: '`loom list` is not a top-level command. Use `loom plugin list` to list installed plugins, or run `loom launch list` if you meant to send "list" as a prompt.',
	remove:
		'`loom remove` is not a top-level command. Use `loom plugin uninstall <name>` to remove a plugin, or run `loom launch remove` if you meant to send "remove" as a prompt.',
	uninstall:
		'`loom uninstall` is not a top-level command. Use `loom plugin uninstall <name@marketplace>` to remove a plugin, or run `loom launch uninstall` if you meant to send "uninstall" as a prompt.',
	marketplace:
		'`loom marketplace` is not a top-level command. Use `loom plugin marketplace <add|remove|update|list>` to manage marketplaces, or run `loom launch marketplace` if you meant to send "marketplace" as a prompt.',
	discover:
		'`loom discover` is not a top-level command. Use `loom plugin discover [marketplace]` to browse available plugins, or run `loom launch discover` if you meant to send "discover" as a prompt.',
	upgrade:
		'`loom upgrade` is not a top-level command. Use `loom plugin upgrade [name@marketplace]` to upgrade plugins, or run `loom launch upgrade` if you meant to send "upgrade" as a prompt.',
	enable:
		'`loom enable` is not a top-level command. Use `loom plugin enable <name@marketplace>` to enable a plugin, or run `loom launch enable` if you meant to send "enable" as a prompt.',
	disable:
		'`loom disable` is not a top-level command. Use `loom plugin disable <name@marketplace>` to disable a plugin, or run `loom launch disable` if you meant to send "disable" as a prompt.',
};

// Sub-actions that make `loom marketplace <sub>` unambiguously a management
// command even when multi-word (the reporter's `loom marketplace add xyz`,
 ...
 *
 * A bare verb (`loom marketplace`) always hints. A multi-word invocation only
 * hints when the arguments follow the documented plugin grammar — a marketplace
 * sub-action (`loom marketplace add …`) or a `name@marketplace` plugin id
 * (`loom uninstall foo@bar`) — so genuine prompts that merely begin with one of
 * these words (`loom list all my files`, `loom upgrade the deps`) still launch.
 *
...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	// (`loom --approval-mode=yolo acp`). `run` dispatches strictly on argv[0], so
