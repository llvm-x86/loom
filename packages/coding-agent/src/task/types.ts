...
export interface TaskItem {
	...
	task?: string;
	/** Per-spawn model override: concrete pattern ("provider/model", optional :thinking suffix), role alias ("@smol"), or ordered fallback chain. Never persisted. */
	model?: string | string[];
...
export interface TaskParams {
	...
	/** The work (flat form). */
	task?: string;
	/** Flat-form per-spawn model override; batch form carries it per item. */
	model?: string | string[];
	/** Caller-provided output schema; its presence overrides the selected agent's schema. */
	outputSchema?: unknown;
	/** Validation behavior for a caller-provided or inherited output schema. */
