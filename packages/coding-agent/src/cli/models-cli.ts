/**
 * `loom models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.loom/models.db`
 ...
 * name doubles as a `loom models <provider>` shortcut.
 */
...
/** `loom models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `loom models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`loom models find` requires a search substring, e.g. `loom models find minimax`\n");
		process.exitCode = 1;
		return;
	}
