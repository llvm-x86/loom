	...
	try {
		if (pluginId) {
			...
			if (flags.scope) {
				console.error(
					chalk.yellow(
						`Warning: --scope is ignored when upgrading all plugins. Use 'loom plugin upgrade <id> --scope ${flags.scope}' to target a specific plugin and scope.`,
					),
				);
			}
...
async function handleInstall(
	...
	for (const spec of packages) {
		...
		if (target.type === "local") {
			...
			// `loom plugin link <path>` so users can use either verb interchangeably.
			if (flags.scope) {
				console.error(
					chalk.yellow(
