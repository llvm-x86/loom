...
```bash
loom auth-broker login              # interactive provider selection
loom auth-broker login anthropic    # login to a specific provider
loom auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
loom auth-broker list               # list supported providers
loom auth-broker logout             # interactive — pick a stored credential to remove
```
