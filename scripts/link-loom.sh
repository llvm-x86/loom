#!/bin/sh
# Install the dev `loom` wrapper into Bun's global bin directory.
#
...
# `packages/coding-agent/scripts/loom`. See that wrapper's header comment for the
...
target=$repo_root/packages/coding-agent/scripts/loom

if [ ! -x "$target" ]; then
	echo "link-loom: target wrapper not found or not executable: $target" >&2
	exit 1
fi
...
mkdir -p "$global_bin"
ln -sfn "$target" "$global_bin/loom"
echo "link-loom: linked $global_bin/loom -> $target"
