...
# Loom Coding Agent Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/llvm-x86/loom/main/scripts/install.sh | sh
#
# Options:
...
INSTALL_DIR="${LOOM_INSTALL_DIR:-${PI_INSTALL_DIR:-$HOME/.local/bin}}"
...
install_via_bun() {
    ...
    echo ""
    echo "✓ Installed loom via bun"
    echo "Run 'loom' to get started!"
}
...
install_binary() {
    ...
    curl -fsSL --connect-timeout 10 --speed-limit 1024 --speed-time 30 "$BINARY_URL" -o "${INSTALL_DIR}/loom"
    ...
    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'loom' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'loom'" ;;
    esac
}
