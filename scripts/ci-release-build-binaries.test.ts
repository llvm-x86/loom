...
describe("Windows release binary target", () => {
	it("builds the generic Windows release asset with the baseline runtime", async () => {
		...
		expect(output).toContain("Building packages/coding-agent/binaries/loom-windows-x64.exe...");
		expect(output).toContain(
			"DRY RUN Bun.build target=bun-windows-x64-baseline outfile=packages/coding-agent/binaries/loom-windows-x64.exe",
		);
